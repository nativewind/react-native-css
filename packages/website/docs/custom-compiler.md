...
`react-native-css` provides a default runtime to apply the styles generated by the compiler. The `withCss()` function will setup Metro to use the `react-native-css` runtime.

```js title=metro.config.js
const { getDefaultConfig } = require("expo/metro-config");
const { withCSS } = require("react-native-css/compiler");
