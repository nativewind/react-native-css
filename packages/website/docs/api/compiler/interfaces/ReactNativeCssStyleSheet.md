...
```ts
optional f: Partial<Record<"", boolean>>;
```
...
```ts
optional k: [string, AnimationKeyframes][];
```
...
```ts
optional s: [string, StyleRuleSet][];
```
...
```ts
optional vr: [string, LightDarkVariable][];
```
...
```ts
optional vu: [string, LightDarkVariable][];
```
