...
test("ancestor attribute flags survive is and where selector compilation", () => {
  registerCSS(`.child { width: 40px; height: 20px; }
    .child:is(.group[data-state="OPEN" i] *) { width: 80px; }
    .child:where(.group[data-state="OPEN" i] *) { height: 60px; }`);
  const subject = (state: string) => (
    <View className="will-change-container group" {...{ dataSet: { state } }}>
      <Child />
    </View>
  );
  render(subject("closed"));
  ...
  screen.rerender(subject("closed"));
  expect(screen.getByTestId("child").props.style).toMatchObject({
    width: 40,
    height: 20,
  });
